...
const response = await fetch(
  `http://localhost:${process.env.PORT || 5000}/api/students/class/${encodeURIComponent(className)}`,
  {
    headers: {
...
console.log(
  'Test query (all records for class):',
...
console.log(
  `Found ${allClassRecords.length} total records for class ${className}`
);
...
console.log(
  'Final query with role-based filters:',
...
console.log(
  `Found ${attendanceRecords.length} matching attendance records`
);
